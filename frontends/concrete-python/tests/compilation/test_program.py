...
def test_empty_module():
    ...
    """
    with pytest.raises(RuntimeError, match="Tried to define a module without any @fhe.function"):
...
def test_compile():
    ...
    configuration = fhe.Configuration(
        p_error=0.1, parameter_selection_strategy="v0", composable=True, verbose=True
    )
    Module.compile(
...
def test_encrypted_execution():
    ...
    inputset = [np.random.randint(1, 20, size=()) for _ in range(100)]
    configuration = fhe.Configuration(
        p_error=0.1, parameter_selection_strategy="v0", composable=True
    )
